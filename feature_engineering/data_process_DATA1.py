    ...
    for trans_idx, trans_feat in tqdm(tmp_df.iterrows()):
        ...
        for length, tname in zip(time_span, time_name):
            ...
            new_df['trans_at_avg_{}'.format(
                tname)] = correct_data['amt'].mean()
            new_df['trans_at_totl_{}'.format(
                tname)] = correct_data['amt'].sum()
            new_df['trans_at_std_{}'.format(
                tname)] = correct_data['amt'].std()
            new_df['trans_at_bias_{}'.format(
                tname)] = temp_amt - correct_data['amt'].mean()
            new_df['trans_at_num_{}'.format(tname)] = len(correct_data)
            new_df['trans_merchant_num_{}'.format(tname)] = len(
                correct_data.merchant.unique())
            new_df['trans_city_num_{}'.format(tname)] = len(
                correct_data.city.unique())
            new_df['trans_category_num_{}'.format(tname)] = len(
                correct_data.category.unique())
...
def k_neighs(
    ...
    if choose_risk:
        ...
        merchant_idxs = neigh_idxs

    return merchant_idxs
